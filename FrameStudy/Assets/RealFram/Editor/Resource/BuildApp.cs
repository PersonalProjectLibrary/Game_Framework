...
public class BuildApp
{
    ...
    static void SaveVersion(string version, string package)
    {
        ...
        using (FileStream fs = new FileStream(savePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
        {
            ...
            {
                all = sr.ReadToEnd();//读取文件中所有东西
                oneLine = all.Split('\r')[0];//只需读取第一行
            }
        }
        //写入
        using (FileStream fs = new FileStream(savePath, FileMode.OpenOrCreate))
        {
            ...
            {
                if (string.IsNullOrEmpty(all)) all = content;//没打包版本号过，就用初始全部信息
                else all = all.Replace(oneLine, content);//打包过，只覆盖第一行，其他信息不被覆盖
                sw.WriteLine(all);
            }
        }
    }
